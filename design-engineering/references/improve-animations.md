...
> Source skill `improve-animations` from [emilkowalski/skills](https://github.com/emilkowalski/skills) — survey a codebase's animation and motion code as a senior motion advisor, then produce a prioritized audit and self-contained implementation plans for other agents to execute. Read-only on source code — it plans improvements, it does not apply them.
...
The rule catalog with precise values lives in [improve-animations-audit.md](improve-animations-audit.md). The plan format lives in [improve-animations-plan-template.md](improve-animations-plan-template.md). Load them when you audit and when you write plans.
...
Audit against the eight categories in [improve-animations-audit.md](improve-animations-audit.md):
...
One plan per selected finding, using [improve-animations-plan-template.md](improve-animations-plan-template.md), written into `plans/` as `NNN-short-slug.md` (monotonic numbering; respect existing plans). Stamp each plan with the current commit (`git rev-parse --short HEAD`).

Write for the weakest executor: exact file paths and current-code excerpts, the exact target values (cubic-beziers, durations, spring configs — pulled from the audit playbook, never approximated), the repo's own conventions with an exemplar, ordered steps, hard scope boundaries, and a verification section including how to *feel-check* the result (slow motion, frame-by-frame, real device for gestures).
