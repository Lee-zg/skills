...
> Source skill `pick-ui-library` from [emilkowalski/skills](https://github.com/emilkowalski/skills) — pick the right library for a given frontend task from a curated, opinionated list. Only runs when explicitly invoked; it does not trigger on its own.
