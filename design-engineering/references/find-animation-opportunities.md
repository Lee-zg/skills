...
> Source skill `find-animation-opportunities` from [emilkowalski/skills](https://github.com/emilkowalski/skills) — search a codebase or UI for places that don't animate but should, and reject everything that shouldn't. Read-only; it proposes motion with exact values, it does not implement it. For fixing existing animations, use `improve-animations` or `review-animations` instead.
